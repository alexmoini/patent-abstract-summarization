...
class CorpusMaskingDataset(Dataset):
    ...
    def __init__(
            self, path, tokenizer,
            pre_masked=False, mask_prob=0.15,
            sequence_length=512, seed=42):
        """
    ...
    def __getitem__(self, index):
        """
        Returns a dictionary containing the input ids,
        attention mask and token type ids
        """
        if self.pre_masked:  # If the data is already masked
            groundtruth_sequence = \
                    self.data.iloc[index]['groundtruth_sequence']
            masked_sequence = self.data.iloc[index]['masked_sequence']

            masked_sequence_tokens = self.tokenizer(
                    masked_sequence, padding='max_length',
                    max_length=self.sequence_length,
                    truncation=True, return_tensors="pt")
            groundtruth_sequence_tokens = self.tokenizer(
                    groundtruth_sequence, padding='max_length',
                    max_length=self.sequence_length,
                    truncation=True, return_tensors="pt")
            # for key in masked_sequence_tokens.keys():
            #     masked_sequence_tokens[
            #         key] = masked_sequence_tokens[key].squeeze(0)
            # for key in groundtruth_sequence_tokens.keys():
            #     groundtruth_sequence_tokens[
            #         key] = groundtruth_sequence_tokens[key].squeeze(0)
            ...
            groundtruth_sequence_tokens = self.tokenizer(
                    sequence, padding='max_length',
                    max_length=self.sequence_length, truncation=True,
                    return_tensors="pt")
            # for key in groundtruth_sequence_tokens.keys():
            #     # delete the first dim of tensor (i.e. (1, N) -> (N)
            #     groundtruth_sequence_tokens[
            #         key] = groundtruth_sequence_tokens[key].squeeze(0)
            # clone input ids tensor
            # (dont want to overwrite our ground truth input ids)
            # copy dict and replace input ids
            masked_sequence_tokens = copy.deepcopy(groundtruth_sequence_tokens)

            masked_sequence_input_ids = self.__random_masking__(
                    masked_sequence_tokens['input_ids'], self.mask_prob)
