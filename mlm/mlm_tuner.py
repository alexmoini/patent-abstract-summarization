...
class MaskedLanguageModelingModel(pl.LightningModule):
    ...
    """
    def __init__(self, embedding_backbone, learning_rate=1e-4, loss=torch.nn.CrossEntropyLoss()):
        """
        ...
        self.fc = torch.nn.Linear(self.model.config.hidden_size,
                                  ...
                                  bias=False)
        self.bias = torch.nn.Parameter(torch.zeros(self.model.config.vocab_size))

        # Need a link between the two variables so that the bias is correctly resized with `resize_token_embeddings`
        self.fc.bias = self.bias
        self.accuracy = tm.Accuracy(task="multiclass", num_classes=self.model.config.vocab_size)
        self.loss = loss
    ...
    def configure_optimizers(self):
        ...
        "lr_scheduler": {
            ...
            "monitor": "train_loss" # If "monitor" references validation metrics, then "frequency" should be set to a multiple of "trainer.check_val_every_n_epoch".
        }
        }
