...
def load_dataset(dataset_dir, tokenizer, pre_masked=False, sequence_length=128):
    dataset = CorpusMaskingDataset(dataset_dir, tokenizer, pre_masked=False, sequence_length=sequence_length)
    return dataset
...
def main():
    ...
    earlystop_callback = pl.callbacks.EarlyStopping(monitor="train_loss", mode="min", patience=3,
